...
export class MultitrackDatastore extends Datastore {
    ...
    public async ingestZipMultitrack(zipFilePath : string, recordingId : string) : Promise<DatastoreFile[] | null> {
        ...
        datastoreFiles.forEach(async f => {
            await this.dbClient.query(`INSERT INTO multitrack_recording_file (recording_id, file_id) VALUES ($1, $2) ON CONFLICT (file_id) DO NOTHING`, [recordingId, f.id]);
        });
    ...
    public async ingestSingleFileForRecording(filePath : string, recordingId : string) : Promise<DatastoreFile | null> {
        ...
        await this.dbClient.query(`INSERT INTO multitrack_recording_file (recording_id, file_id) VALUES ($1, $2) ON CONFLICT (file_id) DO NOTHING`, [recordingId, datastoreFile.id]);
