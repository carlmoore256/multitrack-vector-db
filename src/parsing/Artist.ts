...
import { Artist, PrismaClient, Genre } from "@prisma/client";

export class CambridgeMTArtist implements IDatabaseWriteable<Artist> {
    public id: string;
    ...
    static fromElement(element: HTMLElement): CambridgeMTArtist {
        ...
        const genreElement = artistElement.closest(
            ".c-mtk__genre"
        ) as HTMLElement;
        const genre = CambridgeMTGenre.fromElement(genreElement)
        // const genre = await cambridgeMTGenre.insertIntoDatabase(new PrismaClient());



        const genres = [genre];

        var name = getTextContent(
            artistElement,
            "h4.m-container__title-bar-item span"
        ) as string;
        ...
        const description = getTextContent(
            artistElement,
            ".m-container__title-bar-item strong"
        );

        var links = Array.from(
            artistElement.querySelectorAll(".m-container__header a")
    ...
    async insertIntoDatabase(client: PrismaClient): Promise<Artist | null> {

        // this.genres.forEach(async (g) => {
        //     const genres = await client.genre.upsert({
        //         where: {
        //             name: g.name,
        //         },
        //         create: {
    ...
    //     const resources = (await db.selectOne("artist_resource", {
    //         artistId: id,
    //     })) as IArtistResource[];
    //     const genres = (await db.selectOne("artist_genre", {
    //         artistId: id,
    //     })) as IGenre[];

    //     return new CambridgeMTArtist(
    //         artist.name,
    //         genres.map((g) => new CambridgeMTGenre(g.name, g.subGenres || [])),
    //         artist.description,
...
export function consolidateArtists(
    ...
    artists.forEach((artist) => {
        ...
        if (!(artist.name in consolidated)) {
            ...
            existing.genres = [
                ...existing.genres,
                ...artist.genres.filter((a) =>
                    existing.genres.every((b) => b.id !== a.id)
                ),
            ];

            // Union of links from existing and new artist
            existing.links = Array.from(
                new Set([...(existing.links || []), ...(artist.links || [])])
            );
