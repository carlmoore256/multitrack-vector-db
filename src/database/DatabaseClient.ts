...
export class DatabaseClient {
  ...
  public async connect(create: boolean = false) {
    await this.db.connect();
    await this.parseTables(create);
    this.isConnected = true;
  }

  public async parseTables(create: boolean = false) {
    try {
  ...
  public async runQueryRowsFromFile(
    filename: string,
    basePath = "./sql/queries/"
  ): Promise<any[] | null> {
  ...
  public async resetDialog(table: string | null = null) {
    if (!table) {
      ...
      const choices = Object.values(this.tables).map((table) => ({
        value: table.name,
        name: table.name,
      }));
      // const choices = Array.from(this.tables.values);
      table = await selectPrompt<string>(
        [{ value: "exit", name: "[Exit]" }, ...choices],
    ...
    while (true) {
      ...
      switch (choice) {
        ...
        case "primaryKey":
          ...
          Debug.log(
            `Primary key of table ${tableName} is [${primaryKey}]`,
            LogColor.Red
          );
          break;
        case "exit":
  ...
  public async deleteTables(tableNames: string[] | null = null) {
    if (!tableNames)
      tableNames = Object.values(this.tables).map((table) => table.name);
    const choice = await yesNoPrompt(
      `Are you sure you want to delete data in the following tables? (cascade) \n${tableNames}`
    );
    if (!choice) {
  ...
  public async clearTables(tableNames: string[] | null = null) {
    if (!tableNames)
      tableNames = Object.values(this.tables).map((table) => table.name);
    const choice = await yesNoPrompt(
      "Are you sure you want to truncate/clear every table?"
    );
    if (!choice) {
